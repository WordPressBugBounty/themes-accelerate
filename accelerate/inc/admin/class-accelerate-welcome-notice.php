...
class Accelerate_Welcome_Notice {
	...
	public function welcome_notice() {
		if ( ! get_option( 'accelerate_admin_notice_welcome' ) ) {
	...
	public function import_button_html() {
		$html = '<a class="btn-get-started button button-primary button-hero" href="#" data-name="' . esc_attr( 'themegrill-demo-importer' ) . '" data-slug="' . esc_attr( 'themegrill-demo-importer' ) . '" aria-label="' . esc_attr__( 'Get started with Accelerate', 'accelerate' ) . '">' . esc_html__( 'Get started with Accelerate', 'accelerate' ) . '</a>';
	...
	public function welcome_notice_markup() {
		...
		$dismiss_url = wp_nonce_url(
			...
			'_accelerate_notice_nonce'
		);
		?>
		<div id="message" class="notice notice-success accelerate-notice">
			<a class="accelerate-message-close notice-dismiss" href="<?php echo esc_url( $dismiss_url ); ?>"></a>
			...
			<div class="accelerate-message__content">
				...
				<div class="accelerate-message__text">
					<h2 class="accelerate-message__heading">
						<?php
						printf(
							/* translators: 1: welcome page link starting html tag, 2: welcome page link ending html tag. */
							esc_html__( 'Welcome! Thank you for choosing Accelerate! To fully take advantage of the best our theme can offer please make sure you visit our %1$swelcome page%2$s.', 'accelerate' ),
							'<a href="' . esc_url( admin_url( 'themes.php?page=accelerate-options' ) ) . '">',
							'</a>'
						);
						?>
					</h2>

					<div class="accelerate-message__cta">
						<?php echo $this->import_button_html(); ?>
						<span class="plugin-install-notice"><?php esc_html_e( 'Clicking the button will install and activate the ThemeGrill demo importer plugin.', 'accelerate' ); ?></span>
					</div>
				</div>
			</div>
		</div> <!-- /.accelerate-message__content -->
		<?php
	}
	...
	public function hide_notices() {
		if ( isset( $_GET['accelerate-hide-notice'] ) && isset( $_GET['_accelerate_notice_nonce'] ) ) { // WPCS: input var ok.
			if ( ! wp_verify_nonce( wp_unslash( $_GET['_accelerate_notice_nonce'] ), 'accelerate_hide_notices_nonce' ) ) { // phpcs:ignore WordPress.VIP.ValidatedSanitizedInput.InputNotSanitized
			...
			$hide_notice = sanitize_text_field( wp_unslash( $_GET['accelerate-hide-notice'] ) );
